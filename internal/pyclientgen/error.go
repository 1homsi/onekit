...
func writeErrorFoundation(p printer) {
	...
	p(`    """Single validation violation, matching onekit.http.FieldViolation."""`)
	p("    field: str")
	p(`    description: str = ""`)
	p("")
	p("")
	...
	p(`    """Raised on HTTP 400 when the server returns onekit.http.ValidationError JSON."""`)
	p("    def __init__(")
	p("        self,")
	p("        status: int,")
	p("        body: bytes,")
...
// because populate has a different signature.
