...
// MarshalJSONOnekit method (via an inline interface assertion) rather than calling
...
// generator must forward opts to Bar.MarshalJSONOnekit (from the encoding generator) so
...
func TestCrossFileInt64UnwrapUsesJsonMarshal(t *testing.T) {
	...
	// to Bar.MarshalJSONOnekit. Direct protojson.Marshal(item) would bypass the custom
	...
	t.Run("no protojson.Marshal(item) for cross-file int64 NUMBER items", func(t *testing.T) {
		if strings.Contains(code, "protojson.Marshal(item)") {
			t.Error("cross_int64_service_unwrap.pb.go uses protojson.Marshal(item) directly: " +
				"Bar.MarshalJSONOnekit will be bypassed and int64 NUMBER fields will serialize as " +
				"quoted strings instead of numbers. The unwrap generator should emit an inline " +
				"MarshalJSONOnekit type assertion for every message-typed item.")
		}
	})

	t.Run("MarshalJSONOnekit forwarding present for cross-file int64 NUMBER items", func(t *testing.T) {
		if !strings.Contains(code, "m.MarshalJSONOnekit(opts)") {
			t.Error("cross_int64_service_unwrap.pb.go should emit an inline MarshalJSONOnekit forward " +
				"so Bar.MarshalJSONOnekit is invoked when marshaling Bar items in the unwrap loop")
		}
	})
