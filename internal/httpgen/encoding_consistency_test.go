...
func TestGoGeneratorsProduceIdenticalEnumEncoding(t *testing.T) {
	...
	httpgenNormalized := normalizeGeneratorComment(string(httpgenContent), "onekit-go-http")
	clientgenNormalized := normalizeGeneratorComment(string(clientgenContent), "onekit-go-client")
...
// TestPhase4SuccessCriteria explicitly verifies each Phase 4 success criterion from the roadmap notes.
...
// to allow comparison between onekit-go-http and onekit-go-client output.
...
// divergence (go-client uses UnmarshalJSONOnekit with opts forwarding).
...
// UnmarshalJSON is allowed to differ because go-client uses UnmarshalJSONOnekit
