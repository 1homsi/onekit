	...
	tests := []struct {
		...
		},
		{
			...
			error: ValidationError{
				Service: "UserService",
				Method:  "ListUsers",
				Message: "GET request has fields that are not bound to path or query parameters: [name email]. GET requests cannot have a request body. Either add [(onekit.http.query)] annotations to these fields, include them in the path as variables, or change the HTTP method to POST/PUT/PATCH.",
			},
			mustContain:    []string{"GET request", "not bound", "cannot have a request body", "onekit.http.query"},
			mustNotContain: nil,
		},
	}
