...
func writeTSWSCodecRuntime(p *Printer) {
	...
	p.P("}")
	p.P()
	p.P("export function wsEncodeRawFrame(header: string, raw: Uint8Array[], size: number): ArrayBuffer {")
	...
	p.P("offset += 4;")
	p.P("for (const segment of raw) { view.setUint32(offset, segment.byteLength); offset += 4; }")
	p.P("for (const segment of raw) { out.set(segment, offset); offset += segment.byteLength; }")
	...
	p.P("}")
	p.P()
	p.P("export function wsEncodeMessage<T>(value: T, encode: (v: T) => unknown, split: ((v: T, raw: Uint8Array[]) => T) | undefined): string | ArrayBuffer {")
	p.P("if (split) {")
	p.P("const raw: Uint8Array[] = [];")
	p.P("const header = split(value, raw);")
	p.P("let size = 0;")
	p.P("for (const segment of raw) size += segment.byteLength;")
	p.P("if (size > 0) return wsEncodeRawFrame(JSON.stringify(encode(header)), raw, size);")
	p.P("}")
...
func writeTSRawFuncs(p *Printer, m *onkir.Message) {
	...
	p.P("export function ", p.rawSplitName(m), "(v: ", typeName, ", raw: Uint8Array[]): ", typeName, " {")
	p.P("const c = { ...v };")
	for _, step := range steps {
		...
		default:
			p.P("raw.push(wsTextEncoder.encode(", prop, ` ?? ""));`)
			p.P(prop, ` = "";`)
		}
	}
	p.P("return c;")
