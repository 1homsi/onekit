...
import { encodeResponseNull } from "./types.ts";
...
function main() {
  ...
  const money: Money = { amount_cents: "12345", amount_number: 999, amounts: ["1", "2", "3"] };
  ...
  const holder: StatusHolder = { status: "active", status_num: 1 };
  const holderJSON = JSON.stringify(holder);
  ...
  const ev: Event = { created_at: "2024-01-15T09:30:00Z", unix_at: 1705311000 };
  ...
  // flatten - the type itself mirrors the wire shape, no encode helper needed
  const order: Order = { id: "o1", billing_street: "123 Main", billing_city: "Springfield" };
