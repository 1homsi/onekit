...
// PackageRef identifies another generated TypeScript module that a
...
// different generated TypeScript module than the one currently being
// written, and if so, which module to import for it. A nil PackageResolver
// (the default) treats every type as local, preserving single-module
// generation.
...
// messages (the only thing types.ts ever prints a type name for) and returns
// the distinct set of external modules referenced, sorted by import path.
...
// types of a file's services - the only types client.ts/server.ts ever print
...
// isLocalMessage reports whether m belongs to the module currently being
// generated (true when there's no resolver, i.e. single-module generation).
func isLocalMessage(resolver PackageResolver, m *onkir.Message) bool {
	if resolver == nil {
		return true
	}
	_, ok := resolver.ResolveMessage(m)
	return !ok
}
