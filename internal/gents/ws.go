...
func writeTSWSSharedRuntime(p *Printer) {
	...
	p.P("readonly code: number | undefined;")
	p.P("readonly closeReason: string | undefined;")
	p.P("constructor(code?: number, closeReason?: string) {")
	p.P(`super(code === undefined ? "websocket closed" : "websocket closed (" + code + (closeReason ? ": " + closeReason : "") + ")");`)
	p.P(`this.name = "WSClosedError";`)
	p.P("this.code = code;")
	p.P("this.closeReason = closeReason;")
	p.P("}")
	p.P("}")
	p.P()
...
func writeTSDuplexClass(p *Printer, m *onkir.Method) {
	...
	p.P("private ws: WebSocket;")
	p.P("private maxFrameBytes: number;")
	p.P("private highWaterMark: number;")
	p.P("constructor(ws: WebSocket, maxFrameBytes: number = DEFAULT_MAX_WS_FRAME_BYTES, highWaterMark: number = DEFAULT_WS_HIGH_WATER_MARK_BYTES, maxMessageBytes: number = DEFAULT_MAX_WS_MESSAGE_BYTES) {")
	p.P("this.ws = ws;")
	p.P("this.maxFrameBytes = maxFrameBytes;")
	p.P("this.highWaterMark = highWaterMark;")
