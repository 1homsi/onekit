...
func TestDirectJSONEncodingCompositionConflictFailsGeneration(t *testing.T) {
	...
	tempDir := t.TempDir()
	pluginPath := filepath.Join(tempDir, "protoc-gen-onekit-go-client")

	buildCmd := exec.Command("go", "build", "-o", pluginPath, "./cmd/protoc-gen-onekit-go-client")
	buildCmd.Dir = projectRoot
	if output, buildErr := buildCmd.CombinedOutput(); buildErr != nil {
		t.Fatalf("build go-client plugin: %v\n%s", buildErr, output)
	}
...
service ComboService {
  ...
  rpc PutCombo(Combo) returns (Combo) {
