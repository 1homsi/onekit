...
func writeClientMethod(
	...
	for _, name := range pathFields {
		if field := findField(method.Request, name); field != nil {
...
func writeTypedClientErrors(p *Printer, method *onkir.Method, errorName string) {
	for _, errorType := range method.ErrorTypes {
		...
		variant := PascalCase(strings.TrimSuffix(errorType.Name, "Error"))
		if variant == "" {
			variant = PascalCase(errorType.Name)
		}
		p.P("if status.as_u16() == ", status, " {")
		p.Indent()
		p.P("let error = serde_json::from_slice(&body).map_err(", errorName, "::Decode)?;")
		p.P("return Err(", errorName, "::", variant, "(error));")
		p.Dedent()
		p.P("}")
	}
}
...
func isBodyBearingVerb(verb string) bool {
	...
	case rustVerbPost, rustVerbPut, rustVerbPatch, "query":
		return true
	default:
		return false
	}
