...
func WriteWSServerRuntime(p *Printer, hasWSCorrelation bool) {
	...
	p.P("fn default() -> Self { Self { max_frame_bytes: DEFAULT_MAX_WS_FRAME_BYTES } }")
	p.P("}")
	p.P()
	...
	p.P("let Some(reply) = self.pending.register(id.clone()).await else { return Err(WsCallError::Closed); };")
