...
func WriteWSServerRuntime(p *Printer, hasWSCorrelation bool) {
	...
	p.P("}")
	p.P()
	p.P("impl<K: std::hash::Hash + Eq + Clone + Send + Sync + 'static, E: serde::Serialize + WsCorrelated<K> + WsRawFrame + Send + Sync + 'static, R: Clone + Send + 'static> WsCallSink<K, E, R> {")
	...
	p.P("let value = if E::WS_TIMEOUT { value.ws_with_timeout(timeout.as_millis().max(1) as u64) } else { value };")
	p.P("let reply = self.start(id.clone(), value).await?;")
...
func WriteWSClientRuntime(p *Printer, hasWSCorrelation bool) {
	...
	p.P("seen: std::sync::Arc<std::sync::atomic::AtomicBool>,")
	p.P("assembler: std::sync::Arc<std::sync::Mutex<WsAssembler>>,")
	p.P("max_message_bytes: usize,")
	p.P("_marker: std::marker::PhantomData<(In, Out)>,")
	p.P("}")
	p.P()
	p.P("impl<In, Out> Clone for WsFrameSocket<In, Out> {")
	p.P("fn clone(&self) -> Self { Self { sink: self.sink.clone(), stream: self.stream.clone(), seen: self.seen.clone(), assembler: self.assembler.clone(), max_message_bytes: self.max_message_bytes, _marker: std::marker::PhantomData } }")
	p.P("}")
	p.P()
	p.P("impl<In: serde::Serialize + WsRawFrame + Clone, Out: serde::de::DeserializeOwned + WsRawFrame> WsFrameSocket<In, Out> {")
	p.P("pub fn new(stream: WsStream) -> Self {")
	p.P("use futures_util::StreamExt;")
	p.P("let (sink, stream) = stream.split();")
	p.P("Self { sink: std::sync::Arc::new(tokio::sync::Mutex::new(sink)), stream: std::sync::Arc::new(tokio::sync::Mutex::new(stream)), seen: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false)), assembler: Default::default(), max_message_bytes: DEFAULT_MAX_WS_MESSAGE_BYTES, _marker: std::marker::PhantomData }")
	p.P("}")
	p.P()
	p.P("pub fn with_max_message_bytes(mut self, limit: usize) -> Self {")
	p.P("self.max_message_bytes = if limit == 0 { DEFAULT_MAX_WS_MESSAGE_BYTES } else { limit };")
	p.P("self")
	p.P("}")
	p.P()
	p.P("pub async fn send(&self, value: &In) -> Result<(), String> {")
	p.P("let (binary, data) = ws_encode_frame(value.clone())?;")
	p.P("use futures_util::SinkExt;")
	p.P("if let Some(chunks) = ws_chunks(binary, &data) {")
	p.P("let mut sink = self.sink.lock().await;")
	p.P("for chunk in chunks { sink.send(tokio_tungstenite::tungstenite::Message::Binary(chunk.into())).await.map_err(|error| error.to_string())?; }")
	p.P("return Ok(());")
	p.P("}")
	p.P("let message = if binary { tokio_tungstenite::tungstenite::Message::Binary(data.into()) } else { tokio_tungstenite::tungstenite::Message::text(String::from_utf8(data).map_err(|error| error.to_string())?) };")
	p.P("self.sink.lock().await.send(message).await.map_err(|error| error.to_string())")
	p.P("}")
	p.P()
	...
	p.P("loop {")
	p.P("let message = stream.next().await?;")
	p.P("self.seen.store(true, std::sync::atomic::Ordering::Relaxed);")
	p.P("return match message {")
	p.P("Ok(Message::Text(text)) => {")
	p.P(`if self.assembler.lock().is_ok_and(|a| a.active()) { return Some(Err("malformed chunked message".to_string())); }`)
	p.P("Some(ws_decode_frame::<Out>(false, text.as_bytes()))")
	p.P("}")
	p.P("Ok(Message::Binary(bytes)) => {")
	p.P(`let Ok(mut assembler) = self.assembler.lock() else { return Some(Err("assembler poisoned".to_string())); };`)
	p.P("match assembler.feed(&bytes, self.max_message_bytes) {")
	p.P("Ok(WsPiece::Whole(data)) => Some(ws_decode_frame::<Out>(true, data)),")
	p.P("Ok(WsPiece::Assembled(binary, data)) => Some(ws_decode_frame::<Out>(binary, &data)),")
	p.P("Ok(WsPiece::Pending) => continue,")
	p.P(`Err(code) => Some(Err(format!("chunked message rejected ({code})"))),`)
	p.P("}")
	p.P("}")
	...
	p.P("}")
	p.P()
	p.P("pub async fn ping(&self) -> Result<(), String> {")
	p.P("use futures_util::SinkExt;")
	p.P("self.sink.lock().await.send(tokio_tungstenite::tungstenite::Message::Ping(Default::default())).await.map_err(|error| error.to_string())")
	p.P("}")
	p.P("}")
	p.Blank()
	if !hasWSCorrelation {
...
func writeWSCallSocketType(p *Printer) {
	...
	p.P("}")
	p.P()
	p.P("impl<K: std::hash::Hash + Eq + Clone + Send + Sync + 'static, In: serde::Serialize + WsCorrelated<K> + WsRawFrame + Clone + Send + Sync + 'static, Out: serde::de::DeserializeOwned + WsCorrelated<K> + WsRawFrame + Send + Sync + 'static> WsCallSocket<K, In, Out> {")
	p.P("pub fn new(stream: WsStream, ping_interval: Option<std::time::Duration>, max_message_bytes: usize) -> Self {")
	p.P("let socket = WsFrameSocket::<In, Out>::new(stream).with_max_message_bytes(max_message_bytes);")
	...
	p.P("let reader = tokio::spawn(async move {")
	p.Indent()
	...
	p.Dedent()
	p.P("});")
	p.P("if let Some(every) = ping_interval {")
	p.P("let reader = reader.abort_handle();")
	p.P("let (ping_socket, ping_pending) = (socket.clone(), pending.clone());")
	p.P("tokio::spawn(async move {")
	p.P("let mut ticker = tokio::time::interval_at(tokio::time::Instant::now() + every, every);")
	p.P("let mut awaiting_pong = false;")
	p.P("loop {")
	p.P("ticker.tick().await;")
	p.P("if reader.is_finished() { break; }")
	p.P("if ping_socket.seen.swap(false, std::sync::atomic::Ordering::Relaxed) { awaiting_pong = false; }")
	p.P("if awaiting_pong {")
	p.P("reader.abort();")
	p.P("ping_pending.close_all().await;")
	p.P("ping_socket.close().await;")
	p.P("break;")
	p.P("}")
	p.P("awaiting_pong = true;")
	p.P("if ping_socket.ping().await.is_err() { break; }")
	p.P("}")
	p.P("});")
	p.P("}")
...
func writeRustWSClientMethod(p *Printer, s *onkir.Service, m *onkir.Method) {
	...
	if correlated {
		...
		p.P("Ok(", socketConstructor, "::new(stream).with_max_message_bytes(self.max_ws_message_bytes))")
	}
	p.Dedent()
	p.P("}")
	p.Blank()
...
func writeWSCallSocketCallMethods(p *Printer) {
	...
	p.P("let stamped = if In::WS_TIMEOUT { Some(value.clone().ws_with_timeout(timeout.as_millis().max(1) as u64)) } else { None };")
	p.P("let reply = self.start(id.clone(), stamped.as_ref().unwrap_or(value)).await?;")
	p.P("let mut guard = self.guard(id.clone());")
	p.P("let result = tokio::time::timeout(timeout, reply).await;")
	p.P("guard.on_drop = None;")
	p.P("match result {")
...
func writeWSServerHelpers(p *Printer) {
	...
	p.P("pub max_message_bytes: usize,")
	p.P("pub ping_interval: Option<std::time::Duration>,")
	p.P("}")
	p.P()
	p.P("impl Default for WsServerOptions {")
	p.P("fn default() -> Self { Self { max_frame_bytes: DEFAULT_MAX_WS_FRAME_BYTES, max_message_bytes: DEFAULT_MAX_WS_MESSAGE_BYTES, ping_interval: Some(std::time::Duration::from_secs(30)) } }")
	p.P("}")
	p.P()
...
func writeWSServerReadLoop(p *Printer, method *onkir.Method, requestRef string, correlated bool) {
	...
	p.P("let mut awaiting_pong = false;")
	p.P("let mut assembler = WsAssembler::default();")
	p.P("loop {")
	p.Indent()
	...
	p.P("axum::extract::ws::Message::Text(text) => {")
	p.P(`if assembler.active() { ws_close(&sink, 1007, "malformed chunked message".to_string()).await; break; }`)
	p.P("ws_decode_frame::<", requestRef, ">(false, text.as_bytes())")
	p.P("}")
	p.P("axum::extract::ws::Message::Binary(bytes) => match assembler.feed(&bytes, ws_options.max_message_bytes) {")
	p.P("Ok(WsPiece::Whole(data)) => ws_decode_frame::<", requestRef, ">(true, data),")
	p.P("Ok(WsPiece::Assembled(binary, data)) => ws_decode_frame::<", requestRef, ">(binary, &data),")
	p.P("Ok(WsPiece::Pending) => continue,")
	p.P(`Err(code) => { ws_close(&sink, code, "chunked message rejected".to_string()).await; break; }`)
	p.P("},")
...
func writeWSSinkSend(p *Printer) {
	p.P("impl<E: serde::Serialize + WsRawFrame> WsSink<E> {")
	p.P("pub async fn send(&self, value: E) -> Result<(), String> {")
	p.P("let (binary, data) = ws_encode_frame(value)?;")
	p.P("use futures_util::SinkExt;")
	p.P("if let Some(chunks) = ws_chunks(binary, &data) {")
	p.P("let mut sink = self.sink.lock().await;")
	p.P("for chunk in chunks { sink.send(axum::extract::ws::Message::Binary(chunk.into())).await.map_err(|error| error.to_string())?; }")
	p.P("return Ok(());")
	p.P("}")
	p.P("let message = if binary { axum::extract::ws::Message::Binary(data.into()) } else { axum::extract::ws::Message::text(String::from_utf8(data).map_err(|error| error.to_string())?) };")
	p.P("self.sink.lock().await.send(message).await.map_err(|error| error.to_string())")
	p.P("}")
	p.P("}")
	p.Blank()
