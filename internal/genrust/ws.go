...
func WriteWSClientRuntime(p *Printer, hasWSCorrelation bool) {
	...
	p.P("fn clone(&self) -> Self { Self { sink: self.sink.clone(), stream: self.stream.clone(), seen: self.seen.clone(), _marker: std::marker::PhantomData } }")
	p.P("}")
	p.P()
	...
	p.P("Self { sink: std::sync::Arc::new(tokio::sync::Mutex::new(sink)), stream: std::sync::Arc::new(tokio::sync::Mutex::new(stream)), seen: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false)), _marker: std::marker::PhantomData }")
	p.P("}")
	p.P()
	...
	p.P("Ok(Message::Text(text)) => Some(ws_decode_frame::<Out>(false, text.as_bytes())),")
	p.P("Ok(Message::Binary(bytes)) => Some(ws_decode_frame::<Out>(true, &bytes)),")
...
func writeWSCallSocketType(p *Printer) {
	...
	p.P("pub fn new(stream: WsStream, ping_interval: Option<std::time::Duration>) -> Self {")
...
func writeRustWSClientMethod(p *Printer, s *onkir.Service, m *onkir.Method) {
	...
	if correlated {
		p.P("Ok(", socketConstructor, "::new(stream, self.ws_ping_interval))")
	} else {
...
func writeWSServerReadLoop(p *Printer, method *onkir.Method, requestRef string, correlated bool) {
	...
	p.P("axum::extract::ws::Message::Text(text) => ws_decode_frame::<", requestRef, ">(false, text.as_bytes()),")
	p.P("axum::extract::ws::Message::Binary(bytes) => ws_decode_frame::<", requestRef, ">(true, &bytes),")
