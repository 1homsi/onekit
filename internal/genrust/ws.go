...
func writeWSUpgradeHandler(p *Printer, service *onkir.Service, method *onkir.Method) {
	...
	if correlated {
		...
		p.Indent()
		p.P("Some(id) => match out.pending.resolve(&id, frame).await { Some(frame) => frame, None => continue },")
		p.P("None => frame,")
		p.Dedent()
		p.P("};")
	}
...
func WriteWSServerRuntime(p *Printer, hasWSCorrelation bool) {
	...
	p.P(`let Some(reply) = self.pending.register(id.clone()).await else { return Err("websocket closed".to_string()); };`)
	p.P("if let Err(error) = self.send(value).await { self.pending.cancel(&id).await; return Err(error); }")
...
func writeWSPendingType(p *Printer) {
	...
	p.P("waiters: std::sync::Arc<tokio::sync::Mutex<Option<std::collections::HashMap<K, tokio::sync::oneshot::Sender<T>>>>>,")
	p.P("}")
	p.P()
	...
	p.P("}")
	p.P()
	p.P("pub async fn register(&self, id: K) -> Option<tokio::sync::oneshot::Receiver<T>> {")
	p.P("let mut guard = self.waiters.lock().await;")
	p.P("let waiters = guard.as_mut()?;")
	p.P("let (tx, rx) = tokio::sync::oneshot::channel();")
	p.P("waiters.insert(id, tx);")
	p.P("Some(rx)")
	p.P("}")
	p.P()
	p.P("// resolve hands value back when no call is waiting on id, so the caller")
	p.P("// can still dispatch it as an ordinary inbound frame.")
	p.P("pub async fn resolve(&self, id: &K, value: T) -> Option<T> {")
	p.P("let sender = self.waiters.lock().await.as_mut().and_then(|waiters| waiters.remove(id));")
	p.P("match sender {")
	p.P("Some(sender) => { let _ = sender.send(value); None }")
	p.P("None => Some(value),")
	p.P("}")
	p.P("}")
	p.P()
...
func writeWSCallSocketType(p *Printer) {
	...
	p.P("let frame = match frame.ws_id() {")
	p.Indent()
	p.P("Some(id) => match reader_pending.resolve(&id, frame).await { Some(frame) => frame, None => continue },")
	p.P("None => frame,")
	p.Dedent()
	p.P("};")
