...
func GenerateTypesWithResolver(file *onkir.File, resolver PackageResolver) []byte {
	p := newPrinter(resolver).forFile(file)
	features := collectTypeFeatures(file)

	p.P("// Code generated by onek. DO NOT EDIT.")
	p.P("#![allow(unused_mut, unused_imports, dead_code, deprecated, clippy::all)]")
...
func writeValidationError(p *Printer) {
	...
	p.P("pub struct ", p.validationError, " {")
	p.Indent()
	...
	p.P("}")
	p.Blank()
	p.P("impl std::fmt::Display for ", p.validationError, " {")
	p.Indent()
	...
	p.P("}")
	p.Blank()
	p.P("impl std::error::Error for ", p.validationError, " {}")
	p.Blank()
...
func writeRustDeprecated(p *Printer, deprecated func() (string, bool)) {
	...
	case reason == "":
		p.P("#[deprecated]")
	default:
		p.P("#[deprecated(note = ", strconv.Quote(reason), ")]")
	}
}
...
func writeField(p *Printer, field *onkir.Field) {
	...
	writeRustDeprecated(p, field.Deprecated)
...
func writeValidation(p *Printer, message *onkir.Message) {
	...
	p.Indent()
	p.P("pub fn validate(&self) -> Result<(), ", p.validationError, "> {")
	p.Indent()
	for _, field := range message.Fields {
