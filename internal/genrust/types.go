...
func writeHexDecodeHelper(p *Printer) {
	...
	p.Indent()
	p.P("if value.len() % 2 != 0 { return Err(\"hex value has odd length\".into()); }")
	p.P("(0..value.len()).step_by(2)")
	p.Indent()
	p.P(".map(|index| u8::from_str_radix(&value[index..index + 2], 16).map_err(|error| error.to_string()))")
	p.P(".collect()")
	p.Dedent()
	p.Dedent()
	p.P("}")
