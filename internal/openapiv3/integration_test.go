...
func TestPluginIntegration(t *testing.T) {
	...
	pluginPath := "./protoc-gen-onekit-openapiv3-integration-test"
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := exec.Command("protoc",
				...
				"--onekit-openapiv3_opt=format="+tc.format,
...
func TestPluginErrorHandling(t *testing.T) {
	// Build the plugin binary for testing
	pluginPath := "./protoc-gen-onekit-openapiv3-error-test"
	buildCmd := exec.Command("go", "build", "-o", pluginPath, "../../cmd/protoc-gen-onekit-openapiv3")
	if err := buildCmd.Run(); err != nil {
		t.Fatalf("Failed to build plugin: %v", err)
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			args: []string{
				"--plugin=protoc-gen-onekit-openapiv3=" + pluginPath,
				"--onekit-openapiv3_out=" + t.TempDir(),
				"--proto_path=testdata/proto",
		...
		},
		{
			...
			args: []string{
				"--plugin=protoc-gen-onekit-openapiv3=" + pluginPath,
				"--onekit-openapiv3_out=" + t.TempDir(),
				"--onekit-openapiv3_opt=format=invalid",
				"--proto_path=testdata/proto",
				"testdata/proto/simple_service.proto",
			},
...
func TestPluginFormatOptions(t *testing.T) {
	// Build the plugin binary for testing
	pluginPath := "./protoc-gen-onekit-openapiv3-format-test"
	buildCmd := exec.Command("go", "build", "-o", pluginPath, "../../cmd/protoc-gen-onekit-openapiv3")
	if err := buildCmd.Run(); err != nil {
		t.Fatalf("Failed to build plugin: %v", err)
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.format != "" {
				args = append(args[:2], append([]string{"--onekit-openapiv3_opt=format=" + tc.format}, args[2:]...)...)
			}
...
func TestPluginServiceGeneration(t *testing.T) {
	// Build the plugin binary for testing
	pluginPath := "./protoc-gen-onekit-openapiv3-service-test"
	buildCmd := exec.Command("go", "build", "-o", pluginPath, "../../cmd/protoc-gen-onekit-openapiv3")
	if err := buildCmd.Run(); err != nil {
		t.Fatalf("Failed to build plugin: %v", err)
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := exec.Command("protoc",
				"--plugin=protoc-gen-onekit-openapiv3="+pluginPath,
				"--onekit-openapiv3_out="+tempDir,
				"--onekit-openapiv3_opt=format=yaml",
				"--proto_path=testdata/proto",
				"--proto_path=../../proto",
				"testdata/proto/"+tc.protoFile,
			)
