...
func TestBundleGoldenFiles(t *testing.T) {
	pluginPath := "./protoc-gen-onekit-openapiv3-bundle-test"
...
func TestBundleAlongsidePerService(t *testing.T) {
	pluginPath := "./protoc-gen-onekit-openapiv3-alongside-test"
	buildCmd := exec.Command("go", "build", "-o", pluginPath, "../../cmd/protoc-gen-onekit-openapiv3")
	if err := buildCmd.Run(); err != nil {
		t.Fatalf("Failed to build plugin: %v", err)
	}
	...
	cmd := exec.Command("protoc",
		"--plugin=protoc-gen-onekit-openapiv3="+pluginPath,
		"--onekit-openapiv3_out="+tempDir,
		"--onekit-openapiv3_opt=format=yaml,bundle=true,bundle_title=Alongside",
		"--proto_path=testdata/proto",
		"--proto_path=../../proto",
