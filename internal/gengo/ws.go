...
func writeWSOutType(p *Printer) {
	...
	p.P("binaryFrame, data, err := wsEncode(value)")
...
func writeWSDuplexType(p *Printer, inName, outName string, idField *onkir.Field, reqMessage, respMessage *onkir.Message) {
	...
	if idField == nil {
		...
		p.P("typ, data, err := d.conn.Read(ctx)")
		p.P("if err != nil { return nil, wsReadError(err) }")
		p.P("frame := new(", outName, ")")
		p.P("if err := wsDecode(typ == websocket.MessageBinary, data, frame); err != nil { return nil, fmt.Errorf(\"decode frame: %w\", err) }")
...
func writeWSCorrelatedDuplexMethods(p *Printer, name, inName, outName string, idField *onkir.Field, reqMessage, respMessage *onkir.Message) {
	...
	p.P("for {")
	p.P("typ, data, err := d.conn.Read(context.Background())")
	p.P("if err != nil {")
	...
	p.P("}")
	p.P("frame := new(", outName, ")")
	p.P("if err := wsDecode(typ == websocket.MessageBinary, data, frame); err != nil {")
	p.P(`err = fmt.Errorf("decode frame: %w", err)`)
...
func writeWSRoute(p *Printer, s *onkir.Service, m *onkir.Method) {
	...
	p.P("}")
	p.P("for {")
	p.P("typ, data, err := conn.Read(ctx)")
	if correlated {
