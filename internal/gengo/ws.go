...
func writeWSDuplexType(p *Printer, inName, outName string, idField *onkir.Field, reqMessage, respMessage *onkir.Message) {
	...
	if idField != nil {
		...
		p.P("inboxMu sync.Mutex")
		p.P("inbox []*", outName)
		p.P("inboxSignal chan struct{}")
		p.P("inboxDone bool")
		p.P("readErr error")
...
func writeWSCorrelatedReceive(p *Printer, name, outName string) {
	p.P("func (d *", name, ") notifyInbox() {")
	p.P("select {")
	p.P("case d.inboxSignal <- struct{}{}:")
	p.P("default:")
	p.P("}")
	p.P("}")
	p.P()
	p.P("func (d *", name, ") finishInbox(err error) {")
	p.P("d.inboxMu.Lock()")
	p.P("d.inboxDone, d.readErr = true, err")
	p.P("d.inboxMu.Unlock()")
	p.P("d.notifyInbox()")
	p.P("}")
	p.P()
	p.P("func (d *", name, ") Receive(ctx context.Context) (*", outName, ", error) {")
	...
	p.P("for {")
	p.P("d.inboxMu.Lock()")
	p.P("if len(d.inbox) > 0 {")
	p.P("frame := d.inbox[0]")
	p.P("d.inbox[0] = nil")
	p.P("d.inbox = d.inbox[1:]")
	p.P("more := len(d.inbox) > 0 || d.inboxDone")
	p.P("d.inboxMu.Unlock()")
	p.P("if more { d.notifyInbox() }")
	p.P("return frame, nil")
	p.P("}")
	p.P("done, err := d.inboxDone, d.readErr")
	p.P("d.inboxMu.Unlock()")
	p.P("if done {")
	p.P("d.notifyInbox()")
	p.P("if err == nil { err = io.EOF }")
	p.P("return nil, err")
	p.P("}")
	p.P("select {")
	p.P("case <-d.inboxSignal:")
	p.P("case <-ctx.Done():")
	p.P("return nil, ctx.Err()")
	p.P("}")
	p.P("}")
...
func writeWSCorrelatedDuplexMethods(p *Printer, name, inName, outName string, idField *onkir.Field, reqMessage, respMessage *onkir.Message) {
	...
	p.P("d.inboxSignal = make(chan struct{}, 1)")
	...
	p.P("d.inboxMu.Lock()")
	p.P("d.inbox = append(d.inbox, frame)")
	p.P("d.inboxMu.Unlock()")
	p.P("d.notifyInbox()")
	p.P("}")
	p.P("}")
	p.P()
	writeWSCorrelatedReceive(p, name, outName)
