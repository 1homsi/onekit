...
func writeWSOutType(p *Printer) {
	...
	p.P("binaryFrame, data, raw, err := wsEncode(value)")
...
func writeWSDuplexType(p *Printer, inName, outName string, idField *onkir.Field, reqMessage, respMessage *onkir.Message) {
	...
	if idField == nil {
		...
		p.P("decodeErr := wsDecode(typ == websocket.MessageBinary, data, frame)")
		writeWSBufferRecycle(p, "d.readBuf", "d.readHint")
		p.P("if decodeErr != nil { return nil, fmt.Errorf(\"decode frame: %w\", decodeErr) }")
...
func writeWSRoute(p *Printer, s *onkir.Service, m *onkir.Method) {
	...
	p.P("decodeErr := wsDecode(typ == websocket.MessageBinary, data, frame)")
	writeWSBufferRecycle(p, "readBuf", "readHint")
	p.P("if decodeErr != nil {")
...
func writeWSBufferRecycle(p *Printer, buf, hint string) {
	p.P(hint, " = len(data)")
	p.P("if typ == websocket.MessageBinary || cap(data) > 1<<20 { ", buf, " = nil } else { ", buf, " = data[:0] }")
}
